...
class Ping(object):
    def __init__(self, destination, export,timeout=1000, packet_size=55, own_id=None):
    ...
    def do(self):
        ...
        try: # One could use UDP here, but it's obscure
            ...
            current_socket.bind((self.export,0))
        ...
        if receive_time:
            ...
            #result = "%d bytes from %s: icmp_seq=%d ttl=%d time=%.1f ms"%(packet_size,ip, icmp_header["seq_number"], ip_header["ttl"], delay)
            result = "%.2f"%(delay)
            return delay,float(result)
        else:
            self.print_failed()
...
def verbose_ping(hostname,export,timeout=50, count=1, packet_size=55):
    p = Ping(hostname,export, timeout, packet_size)
    result = p.run(count)
    return result
